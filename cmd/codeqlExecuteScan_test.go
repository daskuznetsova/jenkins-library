...
func TestPrepareCmdForDatabaseAnalyze(t *testing.T) {
	...
	t.Run("No additional flags, no querySuite, sarif format", func(t *testing.T) {
		...
		cmd, err := prepareCmdForDatabaseAnalyze(map[string]string{}, utils, config, "sarif-latest", "target/codeqlReport.sarif")
		assert.NoError(t, err)
		assert.NotEmpty(t, cmd)
	...
	t.Run("No additional flags, no querySuite, csv format", func(t *testing.T) {
		...
		cmd, err := prepareCmdForDatabaseAnalyze(map[string]string{}, utils, config, "csv", "target/codeqlReport.csv")
		assert.NoError(t, err)
		assert.NotEmpty(t, cmd)
		assert.Equal(t, 5, len(cmd))
	...
	t.Run("Custom flags, overwriting threads", func(t *testing.T) {
		...
		cmd, err := prepareCmdForDatabaseAnalyze(customFlags, utils, config, "sarif-latest", "target/codeqlReport.sarif")
		assert.NoError(t, err)
		assert.NotEmpty(t, cmd)
		assert.Equal(t, 8, len(cmd))
...
func TestAppendCodeqlQuery(t *testing.T) {
	...
	t.Run("Empty query", func(t *testing.T) {
		...
		query := ""
		cmd = appendCodeqlQuery(utils, cmd, query, "")
	...
	t.Run("Transform string", func(t *testing.T) {
		...
		cmd = appendCodeqlQuery(utils, cmd, query, `-E "s/^(java|python)-(security-extended\.qls|security-and-quality\.qls)/sap-\1-\2/"`)
		assert.Equal(t, 3, len(cmd))
	...
	t.Run("Transform string", func(t *testing.T) {
		cmd := []string{"database", "analyze"}
		query := "php-security-extended.qls"
		cmd = appendCodeqlQuery(utils, cmd, query, `-E "s/^(java|python)-(security-extended\.qls|security-and-quality\.qls)/sap-\1-\2/"`)
		assert.Equal(t, 3, len(cmd))
